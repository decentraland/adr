...
1. **Expiration Date**:  
   - This specifies a timestamp beyond which the trade is no longer valid. Once this date is reached, the trade can no longer be executed, ensuring that old or unwanted trades expire automatically.

2. **Effective Date**:  
   - The trade can only be executed after this timestamp has passed. This allows for scheduling future trades or delaying the execution of trades until specific conditions are met.

3. **Execution Limits**:  
   - Trades can be configured to be executed a set number of times. This feature is particularly useful for repetitive trades such as Collection Item Minting (Primary Sales).

4. **Authorized Executors**:  
   - Each trade can specify which addresses are allowed to execute it. This means that trades can be limited to specific users or open to anyone, depending on the desired use case.

5. **External Contract Checks**:  
   - The system allows for external smart contracts to be invoked to perform additional checks before the trade is executed. This ensures that complex conditions (such as checking a user's balance, ownership or permissions)

6. **Assets to be Received**:  
   - The trade structure defines the assets that the trade's creator (the signer) expects to receive from the counterparty. These assets can be ERC-20 tokens, ERC-721 (NFT) tokens, or other supported assets. This flexibility allows the system to handle various types of asset swaps.

7. **Assets to be Given in Exchange**:  
   - The trade also specifies which assets the trade's creator will give to the counterparty. Like the assets to be received, this can involve ERC-20, ERC-721 tokens, or a combination of both.
