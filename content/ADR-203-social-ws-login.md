...
In this line, the idea is to revise this solution after Social Service M2 is released and then decide if better Authentication management can be done (like implementing the `3` solution). So, the `.proto` definition used to manage the friendships in the Social Service for M2 is for internal use only, and the release will be marked as beta.
