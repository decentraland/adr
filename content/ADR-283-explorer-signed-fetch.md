...
The Explorer Client runs scenes that interact with external HTTP services. When scenes make authenticated requests on behalf of users, additional context beyond the basic Signed Fetch mechanism is valuable for external services. The Explorer Client includes scene-specific metadata in signed requests to provide:
