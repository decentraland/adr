...
IRealmsModifier.cs describes the functionality necessesary for realms modifiers. It consists on void OnEnteredRealm(bool isCatalyst, AboutResponse realmConfiguration) method
...
- Skybox Modifier (V1 task)
Create a skybox modifier to allow the world creator control its state.
...
- Green Blockers Modifiers
Extra functionality should be added to the current green blockers modifier.
- [] Green blockers should appear 2 parcels away from the scene area

- Loading Screen Modifier (V1 task)
This are required functionalities that should be built on top of the current loading screen when accessing worlds. Requires coordination with the design team.
...
- Teleports
- [] Teleport chat commands. All teleports commands (goto random, crowd, coordinates, home) need to verify if we are on a DAO realm and if necessary change to one before completing the command. Its recommended to solve this on kernel directly.
