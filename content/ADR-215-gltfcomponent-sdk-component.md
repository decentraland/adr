...
Nodes that are not meshes are also affected by this naming convention. If a node is not a mesh, but it has a direct child that is a mesh, then the mesh MUST be invisible. This behavior only affect to the direct  children meshes, not to the children of the children and the rest of the descendants.
